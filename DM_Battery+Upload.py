...
async def scarica_e_unisci_csv(url, cartella_destinazione, file_excel_destinazione):
    ...
    for link in links:
        ...
        logger.info(f"Scaricato DataFrame da {file_csv_url} ({df.shape[0]} righe, {df.shape[1]} colonne)")
        ...
        try:
            ...
            logger.info(f"Ultimo valore per {nome_foglio}: {ultimo_valore}")
            ...
            logger.warning(f"La colonna non è presente nel DataFrame.")
...
async def main():
    ...
    if all_files_updated:
        logger.info("Tutte le schede sono in funzione.")
        ...
        for file_url in csv_file_urls:
            if not await check_remote_file_update(file_url):
                logger.warning(f"Il file {file_url} non è stato aggiornato da più di 16 minuti. Invio messaggio Telegram.")
                bot = Bot(token=TELEGRAM_BOT_TOKEN)
